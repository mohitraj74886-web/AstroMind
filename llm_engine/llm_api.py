...
DEPENDENCY GRAPH:
  [Browser Frontend]
       ...
       ▼
  [llm_api.py — Port 8004]   ←──── gemini (local LLM, no internet needed)
       │
...
INSTALL REQUIREMENTS:
  pip install fastapi uvicorn httpx  langchain-core pydantic

RUNNING:
  uvicorn llm_engine.llm_api:app --reload --port 8004
"""
...
VOICE_MODULE_URL:       str   = os.getenv("VOICE_MODULE_URL",   "http://127.0.0.1:8000")
SLEEP_MODULE_URL:       str   = os.getenv("SLEEP_MODULE_URL",   "http://127.0.0.1:8001")
...
def _create_llm() -> ChatGoogleGenerativeAI:
    """
    WHY no num_predict / stop sequences here:
    Gemini uses max_output_tokens and handles turn-taking natively.
    Temperature stays at 0.15 — same anti-hallucination reasoning applies.
    """
    if not GEMINI_API_KEY:
        raise RuntimeError(
            "GOOGLE_GEMINI_KEY is not set. "
    ...
    return ChatGoogleGenerativeAI(
        ...
        max_output_tokens=300,      # enforces brevity for TTS
...
def get_llm() -> ChatGoogleGenerativeAI:
    ...
    if _llm is None:
        raise RuntimeError(
            ...
            "may have failed.  Check gemini is running and the model is pulled: "
            f"`gemini pull {GEMINI_MODEL}`"
        )
    return _llm
...
async def lifespan(app: FastAPI):
    ...
    try:
        _llm = _create_llm()
        logger.info(f"ChatGoogleGenerativeAI instance created for model '{GEMINI_MODEL}'.")
    except Exception as e:
...
async def health_check() -> HealthResponse:
    ...
    WHY this endpoint exists:
      ...
      - Checks reachability of all three external dependencies (gemini, Voice, Sleep).
      - Does NOT load the model or run inference — just pings each service.
    """
    llm_ok = voice_ok = sleep_ok = False

    try:
        # Ping Gemini by attempting a minimal token generation
        test_llm = get_llm()
        await (test_llm | StrOutputParser()).ainvoke([HumanMessage(content="ping")])
        llm_ok = True
    except Exception:
        llm_ok = False
